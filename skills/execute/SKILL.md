---
name: execute
description: Internal sillok stage skill — enter via the /sillok-execute command or a sillok:workflow handoff; for natural-language intent invoke sillok:workflow instead. Writes the plan from the spec and runs subagent-driven execution.
user-invocable: false
---

# Sillok Execute

You are running the sillok `execute` stage for the sillok.
...
- `sillok:end` — the next stage (push the branch and open the PR)
- `superpowers:writing-plans` → `superpowers:subagent-driven-development` → `superpowers:requesting-code-review` — the chained skills (steps 4, 6, 8); the writing-plans execution-mode handoff is auto-answered `subagent-driven`
- `sillok:verify-gate` — mandatory end-of-plan whole-branch verification (step 8)
