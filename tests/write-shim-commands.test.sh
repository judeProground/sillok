...
echo "test: writes eight shim files into .claude/commands on fresh project"
bash "$SCRIPT" "$TMPDIR_PROJECT" >/dev/null
for cmd in start design execute end story add epic prd; do
  dest="$TMPDIR_PROJECT/.claude/commands/sillok-$cmd.md"
  [[ -f "$dest" ]] || fail "expected $dest to exist"
done
pass "eight shim files written"
